...
exports.SensorNotFoundError = exports.AttackStorageError = exports.AttackValidationError = void 0;
exports.validateSourceIp = validateSourceIp;
exports.validateProtocol = validateProtocol;
exports.validatePort = validatePort;
exports.validateCoordinates = validateCoordinates;
exports.recordAttackEvent = recordAttackEvent;
exports.getAttacksByDateRange = getAttacksByDateRange;
exports.getAttackStatistics = getAttackStatistics;
exports.getTopAttackers = getTopAttackers;
exports.getAttacksBySensor = getAttacksBySensor;
exports.getAttackDetail = getAttackDetail;
exports.getGeoStatistics = getGeoStatistics;
exports.getAttacksByHoneypot = getAttacksByHoneypot;
exports.searchAttacksByIp = searchAttacksByIp;
exports.deleteOldAttacks = deleteOldAttacks;
exports.handleHPFeedsAttackEvent = handleHPFeedsAttackEvent;
...
exports.AttackValidationError = AttackValidationError;
/**
 * Custom error for attack storage failures
 */
class AttackStorageError extends Error {
    constructor(message) {
        super(message);
        this.statusCode = 500;
...
exports.AttackStorageError = AttackStorageError;
/**
 ...
 */
async function recordAttackEvent(sensorUuid, sourceIp, protocol, mongoPayload, port, country, city, latitude, longitude) {
    // Validate inputs
    if (!validateSourceIp(sourceIp)) {
        throw new AttackValidationError(`Invalid source IP address: ${sourceIp}`);
    }
    if (!validateProtocol(protocol)) {
        throw new AttackValidationError(`Invalid protocol: ${protocol}. Must be one of: TCP, UDP, HTTP, HTTPS, SSH, FTP, SMTP, DNS, ICMP, TELNET, RDP, SMB, OTHER`);
    }
    if (port !== undefined && !validatePort(port)) {
        throw new AttackValidationError(`Invalid port number: ${port}. Must be between 1 and 65535`);
    }
    if (latitude !== undefined &&
        longitude !== undefined &&
        !validateCoordinates(latitude, longitude)) {
        throw new AttackValidationError(`Invalid coordinates: lat=${latitude}, lng=${longitude}`);
    }
    ...
    try {
        ...
        const db = (0, mongodb_2.getMongoDB)();
        ...
        const attack = await prisma_1.prisma.attack.create({
            data: {
                sourceIp,
                protocol: protocol.toUpperCase(),
                sensorId: sensor.id,
        ...
        throw new AttackStorageError(`Failed to store attack event: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
async function getAttacksByDateRange(startDate, endDate, filters) {
    // Build where clause
    const where = {
        timestamp: {
            gte: startDate,
    ...
    const total = await prisma_1.prisma.attack.count({
        where: {
            timestamp: {
                gte: startDate,
                lte: endDate,
    ...
    // Get filtered count
    const filtered = await prisma_1.prisma.attack.count({ where });
 ...
 */
async function getAttackStatistics(startDate, endDate) {
    ...
    // Total attacks
    const totalAttacks = await prisma_1.prisma.attack.count({ where });
    // Unique attackers (distinct source IPs)
    const uniqueAttackers = await prisma_1.prisma.attack.findMany({
        where,
        select: { sourceIp: true },
        distinct: ['sourceIp'],
    });
    // Top protocols
    const protocolCounts = await prisma_1.prisma.attack.groupBy({
        by: ['protocol'],
        where,
        _count: {
            protocol: true,
    ...
    // Top countries
    const countryCounts = await prisma_1.prisma.attack.groupBy({
        by: ['country'],
        where: {
            ...where,
            country: { not: null },
    ...
    const honeypotCounts = await prisma_1.prisma.attack.groupBy({
        by: ['sensorId'],
        where,
        _count: {
            sensorId: true,
    ...
    for (const hc of honeypotCounts) {
        ...
        if (sensor) {
            sensorsMap.set(sensor.honeypot, (sensorsMap.get(sensor.honeypot) || 0) + hc._count.sensorId);
 ...
 */
async function getTopAttackers(limit = 10, startDate, endDate) {
    const where = {};
    ...
    // Group by source IP
    const ipCounts = await prisma_1.prisma.attack.groupBy({
        by: ['sourceIp'],
        where,
        _count: {
            sourceIp: true,
    ...
    for (const ipCount of ipCounts) {
        ...
        const protocols = Array.from(new Set(attacks.map((a) => a.protocol))).sort();
 ...
 * @throws SensorNotFoundError if sensor doesn't exist
 */
async function getAttacksBySensor(sensorId, limit = 20, offset = 0) {
 ...
 */
async function getAttackDetail(mongoId) {
    const db = (0, mongodb_2.getMongoDB)();
    const attackEvents = db.collection('attack_events');
    try {
        const attack = await attackEvents.findOne({
            _id: new mongodb_1.ObjectId(mongoId),
        });
        return attack;
    }
    catch (error) {
 ...
 */
async function getGeoStatistics(startDate, endDate) {
    const where = {
        country: { not: null },
        latitude: { not: null },
        longitude: { not: null },
    ...
    for (const cc of countryCounts) {
        ...
        const uniqueIps = await prisma_1.prisma.attack.findMany({
            where: {
                ...where,
                country: cc.country,
            },
 ...
 */
async function getAttacksByHoneypot(honeypotType) {
    ...
    const count = await prisma_1.prisma.attack.count({
        where: {
 ...
 */
async function searchAttacksByIp(sourceIp, limit = 50) {
    if (!validateSourceIp(sourceIp)) {
        throw new AttackValidationError(`Invalid source IP address: ${sourceIp}`);
    }
    const attacks = await prisma_1.prisma.attack.findMany({
 ...
 */
async function deleteOldAttacks(olderThanDays) {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - olderThanDays);
    // Get attacks to delete (need their mongoIds)
    const attacksToDelete = await prisma_1.prisma.attack.findMany({
        where: {
            timestamp: {
                lt: cutoffDate,
            },
    ...
    const mongoIds = attacksToDelete
        .filter((a) => a.mongoId)
        .map((a) => new mongodb_1.ObjectId(a.mongoId));
    ...
    const result = await prisma_1.prisma.attack.deleteMany({
        where: {
            timestamp: {
                lt: cutoffDate,
            },
 ...
 */
async function handleHPFeedsAttackEvent(sensorUuid, channel, payload) {
    ...
    // Record the attack
    await recordAttackEvent(sensorUuid, sourceIp, protocol, payload, port, country, city, latitude, longitude);
}
