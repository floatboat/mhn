...
async function exportRoutes(fastify) {
    ...
    fastify.get('/export/attacks/json', async (request, reply) => {
        const { startTime, endTime, sensorId, sourceIp, limit = 1000 } = request.query;
        const where = {};
    ...
    fastify.get('/export/statistics', async (request, reply) => {
        ...
        const uniqueTargets = await prisma_1.prisma.attack.findMany({
            where,
            distinct: ['port'],
            select: { port: true },
        });
        const protocols = await prisma_1.prisma.attack.groupBy({
            by: ['protocol'],
            where,
            _count: true,
        });
        const countries = await prisma_1.prisma.attack.groupBy({
            by: ['country'],
            where,
            _count: true,
        });
    ...
    fastify.get('/export/heatmap', async (request, reply) => {
        ...
        const heatmapData = Array.from(attacks
            .reduce((map, attack) => {
            if (!attack.country)
                return map;
