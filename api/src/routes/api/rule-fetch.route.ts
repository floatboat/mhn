...
import {
  ...
  getFetchJobStats,
  cancelFetchJob,
} from '../../services/rule-fetcher-job.service';
import {
  FetchJobResultSchema,
  FetchJobListSchema,
  FetchJobStatsSchema,
} from '../../types/rule-fetch.types';

export default async function ruleFetchRoutes(fastify: FastifyInstance) {
  /**
  ...
  fastify.post<{
    ...
    Response: typeof FetchJobResultSchema;
  }>(
    '/rule-fetch/:sourceId',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        // Create fetch job
        const jobId = await createFetchJob(sourceIdNum);

        // Execute immediately
        const result = await executeFetchJob(jobId);
  ...
  fastify.get<{
    ...
    Response: typeof FetchJobListSchema;
  }>(
    '/rule-fetch/:sourceId',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const { jobs, total } = await listFetchJobs(sourceIdNum, limit, offset);
   ...
   */
  fastify.get<{
    Params: { sourceId: string; jobId: string };
  }>(
    '/rule-fetch/:sourceId/:jobId',
    {
      schema: {
        params: {
          type: 'object',
    ...
    async (request, reply) => {
      ...
      try {
        const job = await getFetchJob(jobIdNum);
  ...
  fastify.get<{
    ...
    Response: typeof FetchJobStatsSchema;
  }>(
    '/rule-fetch-stats',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const stats = await getFetchJobStats(sourceIdNum);
