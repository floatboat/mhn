...
import { parseSnortRule } from './rule-parser';
 ...
 */
export function validateRuleFormat(ruleText: string): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  try {
    ...
    if (!isValidAction(parsed.action)) {
      errors.push(
        `Invalid action: ${parsed.action}. Must be one of: alert, log, pass, drop, reject, sdrop`,
      );
    }
    ...
    if (!isValidProtocol(parsed.protocol)) {
      errors.push(
        `Invalid protocol: ${parsed.protocol}. Must be one of: tcp, udp, icmp, ip, http, ftp, tls, smb, dns, dcerpc, ssh, smtp, imap, http2`,
      );
    }
    ...
    if (parsed.classtype && !isValidClasstype(parsed.classtype)) {
      errors.push(
        `Invalid classtype: ${parsed.classtype}. Not a recognized Snort classtype`,
      );
    }
    ...
    errors.push(
      error instanceof Error ? error.message : 'Failed to parse rule',
    );
  }
 ...
 */
export function isValidSid(sid: number): boolean {
 ...
 */
export function getSidCategory(sid: number): string {
  if (sid >= 1 && sid <= 999999) {
 ...
 */
export function validateRuleContent(ruleText: string): boolean {
